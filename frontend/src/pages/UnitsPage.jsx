...
export default function UnitsPage() {
  const fetcher = useCallback((p) => unitService.list(p), []);
  ...
  const save = async (e) => {
    ...
    const body = { name: fd.get('name'), abbreviation: fd.get('abbreviation') };
    try {
      if (modal.row?.id) await unitService.update(modal.row.id, body);
      else await unitService.create(body);
  ...
  const remove = async (id) => {
    if (!(await confirmToast('Hapus satuan?'))) return;
    try {
    ...
    <div>
      ...
      <DataTable
        columns={[
          ...
          { key: 'abbreviation', label: 'Singkatan', sortable: true },
          {
            key: 'a',
            label: '',
            render: (row) => (
      ...
      <Modal open={modal.open} title={modal.row ? 'Edit' : 'Tambah'} onClose={() => setModal({ open: false, row: null })}>
        <form onSubmit={save} className="space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-slate-600">Singkatan</label>
            <input name="abbreviation" required defaultValue={modal.row?.abbreviation} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
          </div>
